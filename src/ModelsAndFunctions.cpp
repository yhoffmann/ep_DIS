...
namespace GBWModel
{
    double T_times_sigma_0 (double bx, double by)
    {
        return exp( -(bx*bx+by*by)*(fmToGeVm1*fmToGeVm1)/(2.0*BG) ); // in GeV2
    }

    double Q_s_squared (double bx, double by)
    {
        return Q_s_0/*GeV*/ * Q_s_0/*GeV*/ * T_times_sigma_0(bx, by)/*1*/; // in GeV2
    }
...
namespace PsiPsi_delta_evaluated
{
    ...
    {
        if (rx*rx+ry*ry < 1e-20) {rx=1e-10; ry=1e-10;} // <-- without this GSL produces an error because Suave evaluates at xx=0 and gsl_sf_bessel_Kn(0,x) is only defined for x>0
    ...
    {
        if (rx*rx+ry*ry < 1e-20) {rx=1e-10; ry=1e-10;} // <-- without this GSL produces an error because Suave evaluates at xx=0 and gsl_sf_bessel_Kn(0,x) is only defined for x>0
        return -A_Q * std::sqrt(2.0*m_Q_c*N_c) * e_Q * e *  gsl_sf_bessel_K0(epsilonFunc(Q,0.5) * std::sqrt(rx*rx+ry*ry)*fmToGeVm1);  // in GeV2 // z = 0.5 for now because of delta function
    }
...
namespace A_coherent_Integrand_Function // in integration: GeV2 -> GeV2 * fm4 = GeV2 * GeVm4 = GeVm2 and then squared after to give GeVm4
{
